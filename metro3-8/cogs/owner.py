...
class Owner(commands.Cog):
	...
	async def premium(self, ctx):
		...
		await ctx.reply(f"Incorrect usage! \n\n``{ctx.prefix}premium give <user>`` -owner only\n``{ctx.prefix}premium revoke <user>`` -owner only\n``{ctx.prefix}premium status [user=None]``",mention_author=False)


	@premium.command(name="give",aliases=["+" , "add"])
	@commands.is_owner()
	async def _give(self, ctx, user : discord.Member):

		data = await self.bot.premium.find(user.id)
		...
		if data is None:
			...
			await ctx.reply(f"Added premium to **{user}**",mention_author=False)
			await ctx.message.add_reaction(self.bot.check)


		else:
			await ctx.message.add_reaction(self.bot.cross)
			return await ctx.reply(f"This person already has premium! \n\nUse ``{ctx.prefix}premium revoke <user>`` to remove it!",mention_author=False)


	@premium.command(name="revoke",aliases=["-", "remove"])
	@commands.is_owner()
	async def _revoke(self, ctx, user : discord.Member):

		data = await self.bot.premium.find(user.id)

		if not data is None:
			await self.bot.premium.delete(user.id)

			await ctx.reply(f"Removed premium to **{user}**", mention_author=False)
			await ctx.message.add_reaction(self.bot.check)

		else:
			await ctx.message.add_reaction(self.bot.cross)
			return await ctx.reply(f"This person doesn't have premium! \n\nUse ``{ctx.prefix}premium give <user>`` to give them premium!", mention_author=False)

	@premium.command(name="status",description="Check if you or someone else has Metro Premium")
	async def _status(self, ctx, user : discord.Member = None):

		if user is None:

			data = await self.bot.premium.find(ctx.author.id)

			if data is None:
				await ctx.message.add_reaction(self.bot.cross)
				return await ctx.reply(f"**{ctx.author}** doesn't have premium -``{ctx.author.id}``", mention_author=False)

			else:
				await ctx.message.add_reaction(self.bot.check)
				return await ctx.reply(
					f"**{ctx.author}** has premium! :tada: -``{ctx.author.id}`` \nUse ``{ctx.prefix}premium perks`` to view perks!",
		...
		if data is None:
			await ctx.message.add_reaction(self.bot.cross)
			return await ctx.reply(f"**{user}** doesn't have premium -``{user.id}``",mention_author=False)

		else:
			await ctx.message.add_reaction(self.bot.check)
			return await ctx.reply(f"**{user}** has premium! :tada: -``{user.id}`` \nUse ``{ctx.prefix}premium perks`` to view perks!",mention_author=False)
