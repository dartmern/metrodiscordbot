    ...
    async def setup_help_pag(self, ctx, entity=None, title=None):
        ...
        for i in range(0, len(filtered_commands), self.cmds_per_page):
            ...
            for cmd in next_commands:
                ...
                subcommand = " *" if hasattr(cmd, "all_commands") else ""

                commands_entry += (
                    f"• **__{cmd.name}__**\n```yaml\nCommand: {signature}\n```\n\n{desc}\n"
                    if isinstance(entity, commands.Command)
                    else f"• **__{cmd.name}__** {subcommand}\n{desc}\n\n"
                )
            pages.append(commands_entry)


        await Pag(title=f"Metro Bot Help!", color=self.bot.aqua, entries=pages, length=1).start(ctx)
    ...
    async def help_command(self, ctx, *, entity=None):
        if not entity:
            ...
            if cog:
                ...
                if command:
                    em = await self.setup_help_pag(ctx, command, command.name)
